...
export class QuizPagePage implements OnInit  {
    ...
    constructor(private quizService: QuizService, private router: Router ,  private route: ActivatedRoute , private modalController: ModalController , public loadingController: LoadingController , private localStorage : LocalStorageService, private util: UtilsService , public popoverController: PopoverController, public  memesService: MemesService) {
    ...
    startQuizWithExistingData() :boolean{
        //todo: when journey added check if http param contains if its from jourbey then set existing data with  current journey data
      ...
      console.log("ngIni Called")
        ...
        this.localStorage.getOnePausedQuiz().subscribe((paused_quiz) => {
            ...
            if (!this.util.isEmptyObject(paused_quiz)){
                ...
                this.questions = this.start_with_existing_data ? this.existing_data.questions : []

                console.log(this.questions)

                this.paused = this.start_with_existing_data ? false : false

                this.showLoadingBar().then(
                    (data)=>{


                        this.current_page = this.start_with_existing_data ? this.existing_data.current_index : 0
                        this.quiz_config = this.start_with_existing_data ? this.existing_data.quiz_config : {
                            category : '' ,
                            difficulty : '' ,
                            amount : 0
                        }
                        ...
                        if(!this.start_with_existing_data){
                            ...
                            this.questions$ = this.route.paramMap.pipe(
                                switchMap((params:any) => {
                                    ...
                                    return this.quizService.fetchQuizQuestions(this.quiz_config.category , this.quiz_config.difficulty , this.quiz_config.amount)
                            ...
                            this.questions$.subscribe((data)=> {
                                    ...
                                    this.questions.map((question)=>{
                                        question.options = this.reArrangeOptions([...question.incorrect_answers , question.correct_answer ])
                                    } )
                        ...
                        this.quiz_time = this.start_with_existing_data ? this.existing_data.quiz_config.amount * 10 : this.quiz_config.amount * 10
                        this.countdown = this.start_with_existing_data ? this.existing_data.countdown :  this.quiz_config.amount * 10
    ...
    countdownController(operation) {
       ...
       this.counter = setInterval(()=>{
           ...
           if (this.countdown > mid_time) this.progress_color = '#278A09'
           else if (this.countdown < mid_time && this.countdown > quater_time) this.progress_color = '#BF9513'
           else this.progress_color = '#A80B10'
    ...
    async submitQuiz(){
       ...
       correct_answers_count: correct_answers_count ,
        ...
        review_modal.onDidDismiss().then(
            (res)=>{
                console.log(res)
      ...
      let paused_meta = {
       ...
       const pause_quiz_modal = await this.popoverController.create({
           ...
           showBackdrop :true
            ...
            pause_quiz_modal.onDidDismiss().then(
                (res)=>{
                    console.log(res)
