...
export class QuizLandingPage implements OnInit {
 ...
 paused_quiz
  ...
  constructor(private localStorage: LocalStorageService, private util: UtilsService, private router: Router, private  route: ActivatedRoute , private quizService: QuizService) { }


  ngOnInit() {

   this.localStorage.getPausedQuiz().subscribe(
       (paused_quiz)=>{
...
*/
  ...
  resumePausedQuiz(){
   ...
   this.quizService.setInstantStartWithPausedQuiz(false)
    ...
    this.router.navigate([`/quiz-page/${this.paused_quiz.quiz_config.category}/${this.paused_quiz.quiz_config.difficulty}/${this.paused_quiz.quiz_config.amount}?testes=`+Math.random() * 10 ]   );
    ...
    submitQuizConfiguration(){
        ...
        if (!this.quiz_config.category || !this.quiz_config.difficulty || this.quiz_config.amount < 1 ) {
        ...
        this.router.navigate([`/quiz-page/${this.quiz_config.category}/${this.quiz_config.difficulty}/${this.quiz_config.amount}` ] );
